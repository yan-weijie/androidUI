...
public class FollowCheckCaseCase {
	...
	@BeforeClass
	public void setUp() {
		LOG.info("-------------------------  case30-病例-关注/个人中心查看关注/取消关注2 -------------------------");
	...
	public void tearDown() {	
		...
		LOG.info("-------------------------  已完成: case30-病例-关注/个人中心查看关注/取消关注 -------------------------\n\n\n" );
	}
...
//		cl.deleteCollection(Dom4jXml.getUserValue("loginPhoneUser"));
	...
	public void checkfollowCase() {
		LOG.info("个人中心，验证关注的病例");
		IndexPageElements.gotoPer();
		
		PerCenterPageElements.checkCollection("关注", caseName,"病例");
	}
	...
	public void deletefollowCase() {
		...
		if(Utils.isElementExist(driver, By.id(PerCenterPageElements.FTV_TITLE_LIST))) {
			...
			if(caseName.substring(0, 3).equals(name.substring(0, 3))) {
				LOG.error("找到关注的资源：\""+caseName+"\"，取消关注失败！");
