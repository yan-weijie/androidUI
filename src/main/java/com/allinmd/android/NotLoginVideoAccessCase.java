...
public class NotLoginVideoAccessCase {
	private static AndroidDriver<AndroidElement> driver;
	private static final Logger log = Logger.getLogger(NotLoginVideoAccessCase.class);
	
	@BeforeClass
	public void setUp() {
		log.info("-------------------------  case14-未登录-视频终端页权限判断  -------------------------");
	...
	public void tearDown() {	
		...
		log.info("-------------------------  已完成: case14-未登录-视频终端页权限判断  -------------------------\n\n\n" );
	}
	...
	public void notLoginVideoAccess() {
		...
		//点击收藏按钮
		Utils.waitElement(driver, By.id(TerminalPageElements.collection_btn), 10);
		driver.findElement(By.id(TerminalPageElements.collection_btn)).click();
		
		if(LoginPageElements.allmdBtnIsExist()) {
			log.info("未登录视频终端页权限验证：true");
		}else {
			Assertion.assertEquals(false, true, "未登录视频终端页权限验证:false");
		}
	}
